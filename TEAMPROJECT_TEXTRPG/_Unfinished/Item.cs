namespace TEAMPROJECT_TEXTRPG._Unfinished
{
    ...
    internal class EquipItem : Item
    {
        ...
        public EquipItem(int itemIndex, string itemName, int itemType, int itemStat, int itemStatValue, string itemExplanation, bool isEquip)
        {
    ...
    internal class UseItem : Item
    {
        ...
        public UseItem(int itemIndex, string itemName, int itemStat, int itemStatValue, string itemExplanation)
        {
            this.itemIndex = itemIndex;
            this.itemName = itemName;
            this.itemStat = itemStat;
    ...
    internal class EtcItem : Item
    {
        ...
        public EtcItem(int itemIndex, string itemName, string itemExplanation)
        {
            this.itemIndex = itemIndex;
            this.itemName = itemName;
            this.itemExplanation = itemExplanation;
    ...
    internal class Items
    {
        ...
        public Items()
        {
            ...
            equipItems.Add(new EquipItem(0, "", 0, 0, 0, "(비어있음)", false)); // 기본 값
            equipItems.Add(new EquipItem(1, "낡은 검", 1, 1, 5, "낡은 검이다. 당근에서 싸게 거래했다.", false));
            equipItems.Add(new EquipItem(2, "냄비 뚜껑", 2, 2, 2, "냄비 뚜껑이다. 붉은 얼룩이 묻어있다.", false));

            // 소비 Item 리스트에 소비 Items 생성
            useItems.Add(new UseItem(0, "", 0, 0, "(비어있음)")); // 기본 값
            useItems.Add(new UseItem(1, "체력 회복 포션", 1, 30, "체력을 30 회복할 수 있는 포션이다."));
            useItems.Add(new UseItem(2, "마나 회복 포션", 2, 20, "마나를 20 회복할 수 있는 포션이다."));

            // 기타 Item 리스트에 기타 Items 생성
            etcItems.Add(new EtcItem(0, "", "(비어있음)")); // 기본 값
            etcItems.Add(new EtcItem(1, "작은 철 조각", "'미니언'의 잔해이다. 미니언이 휘두르던 무기의 파편인 것 같다.")); // '미니언'이 드랍할 전리품
            etcItems.Add(new EtcItem(2, "빛나는 조각", "'공허충'의 잔해이다. 기묘한 보라빛으로 빛나고 있다.")); // '공허충'이 드랍할 전리품
            etcItems.Add(new EtcItem(3, "무언가의 손잡이", "'대포 미니언'의 잔해이다. 미니언이 몰던 대포의 조종간인 것 같다.")); // '대포 미니언'이 드랍할 전리품
            etcItems.Add(new EtcItem(4, "작은 돌 조각", "'돌거북'의 잔해이다. 왠지 미안해진다..")); // '돌거북'이 드랍할 전리품
            etcItems.Add(new EtcItem(5, "돌 조각", "'고대 돌거북'의 잔해이다. 돌도끼로 사용해도 될 정도로 단단하다.")); // '고대 돌거북'이 드랍할 전리품
            etcItems.Add(new EtcItem(6, "푸르스름한 깃털", "'칼날부리'의 잔해이다. 생각보다 부드럽고 연하다.")); // '칼날부리'가 드랍할 전리품
        }
    }
